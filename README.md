# TwitchPointMiner Streaming ETL Pipeline
![ETL diagram](https://github.com/jcw024/Twitch-Channel-Points-Miner-v2/blob/master/TwitchPointMinerETL.png)

This is a fork of the twitch channel points miner repository that streams data from the point miner into a redshift database.

## Background
Twitch is an online platform for content creators to livestream their videos (primarily gaming videos). During these livestreams, viewers can interact with the streamer through chat, donations, or emotes. Over time, viewers can earn "points" which can be redeemed for emotes, highlighted chat messages, song requests, and sometimes game requests with the streamer. 

The twitch channel points miner app enables users to collect (or "mine") these points passively without requiring the user to view the livestreams themselves. Users can collect points as soon as streamers go live without having to manually monitor streaming schedules, keep track of notifications, or visit Twitch.tv in their browser.

One feature of the twitch channel points miner is the ability to participate in prediction events. Prediction events allow viewers to stake their points on one of two possible future outcomes (i.e. whether or not the streamer will win their game).

I wanted to see if the data behind these predictions could be stored and analyzed to determine optimal prediction strategy for the twitch channel points miner app to use. For the sake of time, I scoped the statement of work to the building of the ETL pipeline without the analysis of the data.

## Project Overview
Here's an overview of what I did to build the pipeline:
1. I modified the twitch channel points miner source code to merge the app's prediction with the true result of each prediction event.
2. I also made some modifications to the app to allow data to be sent to a kafka broker. 
3. I hosted the twitch channel points miner in an EC2 instance where the data would then be sent to a kafka broker hosted on AWS MSK.
4. I created a spark streaming application which would consume the messages from the kafka broker and write the data to a redshift cluster.

I was able to test and debug the pipeline with sample data generated by the twitch channel point miner. Ultimately, the pipeline was successful and live prediction data was able to be loaded into the redshift cluster with almost no delay from the data generation.

The system can be viewed in the ETL diagram above.

The remainder of the README is the original README content for the twitch channel points miner app.
